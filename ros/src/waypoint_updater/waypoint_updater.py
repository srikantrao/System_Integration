...
class WaypointUpdater(object):
    ...
    def distance(self, waypoints, wp1, wp2):
        ...
        :param waypoints: List of waypoints based on the base_waypoints
        :param wp1: Start Waypoint Index
        :param wp2: Stop Waypoint Index
        :return: return the distance(on ground) between the two waypoints
        """
        wlen = len(waypoints)
        dist = 0
        dl = lambda a, b: math.sqrt((a.x - b.x) ** 2 + (a.y - b.y) ** 2 + (a.z - b.z) ** 2)
        for i in range(wp1, wp2 + 1):
            dist += dl(waypoints[i % wlen].pose.pose.position, waypoints[(i+1) % wlen].pose.pose.position)
        return dist
